	...
	,	getContext: function()
		{
			...
			// addition FIX sales order entered from backend - display Order Detail information
			var TT_OrderTypeName = '';
			if(_.findWhere(Configuration.get('transactionRecordOriginMapping'), { origin: this.model.get('origin') || null })){
				TT_OrderTypeName = _.findWhere(Configuration.get('transactionRecordOriginMapping'), { origin: this.model.get('origin') || null }).name
			}else{
				TT_OrderTypeName = 'Purchase'
			}

			var TT_OrderTypeName2 = '';
			if(_.findWhere(Configuration.get('transactionRecordOriginMapping'), { origin: this.model.get('origin') || null })){
				TT_OrderTypeName2 = _.findWhere(Configuration.get('transactionRecordOriginMapping'), { origin: this.model.get('origin') || null }).detailedName
			}else{
				TT_OrderTypeName2 = 'Purchase'
			}
			// END addition FIX sales order entered from backend - display Order Detail information
			...
			return {
				...
				,	originName: TT_OrderTypeName
					//@property {String} title
				,	title: TT_OrderTypeName2
